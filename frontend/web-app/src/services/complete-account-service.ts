...
const completeAccountService = {
    ...
    verifyToken: async (token: string): Promise<Partial<Account>> => {
        ...
        sessionStorage.setItem("completion_session_token", response.session_token);
        sessionStorage.setItem("completion_user_id", response.user_id.toString());
    ...
    completeAccount: async (accountData: Partial<Account>): Promise<void> => {
        const sessionToken = sessionStorage.getItem("completion_session_token");

        if (!sessionToken) {
            throw new Error("No session token found. Please restart the account completion process.");
        }
        ...
        sessionStorage.removeItem("completion_session_token");
        sessionStorage.removeItem("completion_user_id");
    },
